# Hi there, I'm Hermona! 👋
**AI Engineer | Data Engineer | Data Analyst**

I specialize in turning complex data into actionable insights and building intelligent systems that scale. I bridge the gap between rigorous statistical analysis and cutting-edge machine learning engineering.

---

### 🛠️ My Tech Stack

| Category | Tools & Technologies |
| :--- | :--- |
| **Artificial Intelligence** | Python, TensorFlow, PyTorch, Scikit-learn, LangChain, HuggingFace |
| **Data Analysis** | SQL, Pandas, NumPy, Matplotlib, Seaborn |
| **Engineering/DevOps** | Git, Docker, Flask/FastAPI, AWS, Linux, CI/CD |
| **Data Science** | Statistical Modeling, Hypothesis Testing, Feature Engineering, EDA |

---

### 📊 GitHub Stats
![Hermona's GitHub Stats](https://github-readme-stats.vercel.app/api?username=Hermona42&show_icons=true&theme=tokyonight)
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=Hermona42&layout=compact&theme=tokyonight)

---

### 📫 Connect with me
- 💼 [LinkedIn](https://www.linkedin.com/in/hermona/)
- 📧 [Email](mailto:hermonaaddisu@gmail.com)
- ✍️ [Personal Blog](https://medium.com/@hermonaaddisu)

---
*“In God we trust, all others must bring data.” – W. Edwards Deming*
